...
export const handler: Handler = async (event: EventBridgeEvent<string, IntegrationEvent>) => {
  ...
  try {
    ...
    switch (detail.eventType) {
      case EventTypes.AssetRegistered:
        if (assetTable) {
          await doc.send(new PutCommand({
      ...
      case EventTypes.WorkflowExecutionStarted:
        ...
        if (workflowTable) {
          const wfName = detail.payload.workflowName as string;
          await doc.send(new PutCommand({
      ...
      case EventTypes.WorkflowExecutionCompleted:
        ...
        if (workflowTable) {
          const wfName = (detail.payload.workflowName as string) ?? 'default';
          await doc.send(new UpdateCommand({
            TableName: workflowTable,
      ...
      case EventTypes.SnapshotCreated:
        await doc.send(new UpdateCommand({
          TableName: executionTable,
          Key: { PK: tenantPk, SK: execSk },
      ...
      case EventTypes.WebhookDelivered:
        await doc.send(new UpdateCommand({
          TableName: executionTable,
          Key: { PK: tenantPk, SK: execSk },
      ...
      case EventTypes.ExecutionCancelled:
        await doc.send(new UpdateCommand({
          TableName: executionTable,
          Key: { PK: tenantPk, SK: execSk },
      ...
      case EventTypes.ExecutionTimedOut:
        await doc.send(new UpdateCommand({
          TableName: executionTable,
          Key: { PK: tenantPk, SK: execSk },
      ...
      case EventTypes.ExecutionRetried:
        await doc.send(new UpdateCommand({
          TableName: executionTable,
          Key: { PK: tenantPk, SK: execSk },
